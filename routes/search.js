...
module.exports = function searchProducts () {
  return ({ query }, res, next) => {
    ...
    models.sequelize.query('SELECT * FROM Products WHERE ((name LIKE \'%' + criteria + '%\' OR description LIKE \'%' + criteria + '%\') AND deletedAt IS NULL AND on_offer = False) ORDER BY name')
      .then(([products, query]) => {
